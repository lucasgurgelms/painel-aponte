...
export async function buscarLancamentos({ dataInicio, dataFim, regime = 'caixa' }) {
  ...
  // regime 'caixa' = data de pagamento/recebimento; 'competencia' = vencimento.
  // O nome do parâmetro de data é ajustável conforme o endpoint real.
  while (true) {
    ...
    lancamentos.push(...itens);

    const total = (resp && (resp.total_paginas || resp.totalPages)) || 1;
    if (pagina >= total || itens.length === 0) break;
    pagina++;
  }

  return lancamentos.map(normalizarLancamento);
}
...
function normalizarLancamento(l) {
  const valor = Number(
    l.valor ?? l.valor_pago ?? l.valor_recebido ?? l.amount ?? 0
  );
  const tipo = norm(l.tipo_operacao ?? l.tipo ?? l.operationType ?? '');
  const ehCredito = tipo.includes('credito') || tipo.includes('receita') || valor > 0;

  return {
    data: l.data_movimento ?? l.data ?? l.dueDate ?? l.data_vencimento ?? null,
