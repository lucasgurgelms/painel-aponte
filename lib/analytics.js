...
// ---------------------------------------------------------------------------
// Busca de lançamentos financeiros num intervalo de datas.
// A API v2 da Conta Azul pagina; aqui percorremos todas as páginas.
// O endpoint exato e os campos serão confirmados com o payload real — esta
// função isola essa dependência num ponto só.
// ---------------------------------------------------------------------------
export async function buscarLancamentos({ dataInicio, dataFim, regime = 'caixa' }) {
  ...
  return [
    ...receitas.map(l => normalizarLancamento(l, true)),
    ...despesas.map(l => normalizarLancamento(l, false)),
  ];
}
...
async function buscarEventos(tipo, { dataInicio, dataFim, regime }) {
  ...
  // regime 'competencia' filtra por vencimento; 'caixa' por pagamento/recebimento.
  const filtroData = regime === 'competencia'
    ? { data_vencimento_de: dataInicio, data_vencimento_ate: dataFim }
    : { data_pagamento_de: dataInicio, data_pagamento_ate: dataFim };
...
// Padroniza um lançamento cru da API para o formato interno que o motor usa.
// Tolera variações de nome de campo (a API pode usar grafias diferentes).
// ehCredito vem da ORIGEM (contas a receber = crédito; a pagar = débito).
// Os nomes de campo abaixo são tolerantes e serão confirmados com o payload
// real de um lançamento (via /api/contaazul/inspecionar) quando a conta certa
// estiver conectada.
function normalizarLancamento(l, ehCredito) {
  const valor = Number(
    l.valor ?? l.valor_pago ?? l.valor_recebido ?? l.valor_total ?? l.amount ?? 0
  );

  return {
    data: l.data_pagamento ?? l.data_vencimento ?? l.data_movimento ?? l.data ?? l.dueDate ?? null,
    descricao: l.descricao ?? l.description ?? '',
    categoria: l.categoria?.nome ?? l.categoria ?? l.category ?? '',
    // centros de custo: pode vir como lista (rateio) ou único.
...
function extrairCentrosCusto(l) {
  const cc = l.centros_custo ?? l.centrosCusto ?? l.centro_custo ?? l.costCenters ?? l.centro_de_custo;
  if (Array.isArray(cc)) {
...
// ---------------------------------------------------------------------------
// Totais simples (cards da visão geral).
// ---------------------------------------------------------------------------
