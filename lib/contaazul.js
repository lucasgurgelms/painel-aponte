...
async function refreshAccessToken(refreshToken) {
  ...
  if (!res.ok) {
    const txt = await res.text();
    if (res.status === 400 && /invalid_grant/.test(txt)) {
      throw new Error('Conexão com o Conta Azul expirou. Reconecte a conta pelo link de setup (/api/auth/start).');
    }
...
// Garante UM refresh por vez: se várias chamadas paralelas pegam o token
// expirado ao mesmo tempo, todas compartilham a MESMA renovação. Sem isso, os
// refreshes simultâneos podem invalidar o refresh token (rotação) e derrubar a conexão.
let refreshEmAndamento = null;
function renovarComLock(refreshToken) {
  if (!refreshEmAndamento) {
    refreshEmAndamento = refreshAccessToken(refreshToken).finally(() => { refreshEmAndamento = null; });
  }
...
export async function getValidAccessToken() {
  ...
  return await renovarComLock(data.refresh_token); // expirado → renova (1 por vez)
}
...
export async function caFetch(path, { method = 'GET', query, body } = {}) {
  ...
  if (res.status === 401) {
    // token virou inválido no meio do caminho — força refresh (com lock) e tenta 1x mais
    const { data } = await supabase
      .from('ca_tokens').select('refresh_token').eq('account_key', ACCOUNT_KEY).single();
    token = await renovarComLock(data.refresh_token);
